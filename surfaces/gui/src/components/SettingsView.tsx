...
import i18n, { useLocalePref } from "../i18n";
...
export function SettingsView({
  ...
  const tabs = personas ? SET_TABS : SET_TABS.filter((entry) => entry.key !== "personas");
...
const voiceError = (error: unknown) =>
  error instanceof Error ? error.message : typeof error === "string" ? error : i18n.t("Voice Input could not complete that action.");
        ...
        <div className="space-y-4">
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{t("Whisper Base · Multilingual")}</div>
    ...
    <section>
      ...
      <div className={CARD + " p-4 mb-4"}>
        ...
        <div className="seg mt-2.5" role="radiogroup" aria-label={t("Language")}>
          {(["auto", "en", "zh-CN"] as const).map((p) => (
            <button key={p} className={p === locale ? "active" : ""} onClick={() => setLocale(p)}>
              {p === "auto" ? t("Auto") : p === "en" ? t("English") : "中文"}
            </button>
          ))}
        </div>
        <div className={FIELD_HELP}>{t("Auto matches your system language.")}</div>
      </div>
                ...
                <div className="text-[11.5px] text-muted mt-0.5">
                  {workspace.requested_commands.length
                    ? workspace.requested_commands.length === 1
                      ? t("{{count}} project command allowance", { count: workspace.requested_commands.length })
