...
export function PersonasTab({ onOpenPersona }: { onOpenPersona?: (id: string) => void }) {
  ...
  const install = async () => {
    ...
    if (r.personas) setPersonas(r.personas);
    setMsg(t("Installed {{count}} persona(s) — review and enable below.", { count: (r.consent || []).length }));
    setSrc("");
  };
            ...
            {confirmOff === p.id && (
              ...
              >
                <span className="min-w-0">
                  {liveCount(p.id) === 1
                    ? t("Disabling archives its {{count}} conversation — they stay available under “Show archived”.", { count: liveCount(p.id) })
