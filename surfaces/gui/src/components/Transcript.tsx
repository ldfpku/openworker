...
function approvalChip(resolved: ApprovalDecision | undefined) {
  ...
  return (
    <span
      ...
      title={
        resolved
          ? i18n.t("approved · {{decision}}", { decision: resolved.replace(/_/g, " ") })
          : i18n.t("approved")
      }
    >
      {i18n.t("✓ approved")}
      ...
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-lg hover:bg-paper" data-testid="turn-step">
        ...
        {!!tool.standingRule && (
          <span
            ...
            title={t("Auto-allowed by this automation's standing approval: {{rule}}. Revoke on its Automations page.", {
...
function TurnGroup({
  ...
  const stepsLabel =
    nSteps === 1 ? t("{{n}} step", { n: nSteps }) : t("{{n}} steps", { n: nSteps });
              ...
              >
                {item.undone ? (
                  <span data-testid="memory-notice-undone">
                    {item.previous ? t("Okay — put back the way it was.") : t("Okay — forgotten.")}
                  </span>
                ) : (
                  <>
                    <span className="min-w-0">
                      <span className="font-medium">
                        {item.previous ? t("I've updated what I remember") : t("I'll remember that")}
                      </span>
