...
export function MemorySection() {
  ...
  const toggleEnabled = async () => {
    ...
    setToggleMsg(
      next.enabled
        ? t("Details you share from future conversations will be remembered, so I can be more helpful over time.")
        : t("I'll stop remembering new things about you. What I already know is kept and still used — delete anything below you'd rather I forget."),
    );
  };
  ...
  if (!settings || entries === null)
    return <div className="text-[13px] text-muted">{t("Loading…")}</div>;

  return (
    <section>
      <PanelHead
        title={t("Memory")}
        sub={t("Your coworkers can remember useful things about you between conversations. Everything they know is listed here.")}
      />

      {/* On/off — one switch, no other setup (§5.4). */}
      <div className={CARD + " p-4 mb-4"} data-testid="memory-toggle-card">
        <div className="flex items-center gap-3">
          <Toggle checked={settings.enabled} onChange={toggleEnabled} title={t("Remember new things about you")} />
          <div className="min-w-0 flex-1">
            <div className={FIELD_LABEL}>{t("Remember new things about me")}</div>
            <div className="text-[12px] text-muted mt-0.5">
              {t("Lasting preferences you mention in chat get saved and used in future conversations — you'll see a small note each time, with one-tap Undo. Turning this off stops new saves; anything already below is still used until you delete it.")}
            </div>
          </div>
        </div>
        {toggleMsg && (
      ...
      <div className={CARD + " p-4 mb-4"} data-testid="memory-list-card">
        <div className="flex items-center gap-2">
          <div className={FIELD_LABEL + " flex-1"}>{t("What I've learned about you")}</div>
          {entries.length > 0 && (
            <button
        ...
        <div className={FIELD_HELP}>
          {t("Saved automatically from your conversations. Fix anything that's wrong — or delete it. Edits and deletions apply to new conversations; ones you already have open keep what they knew when they started.")}
        </div>
        {listMsg && (
        ...
        {entries.length === 0 ? (
          !listMsg && (
            <div className="text-[12px] text-muted mt-3" data-testid="memory-empty">
              {t('Nothing yet. When you mention a lasting preference in chat — or say "remember that…" — it will show up here.')}
            </div>
          )
        ) : (
...
function UserRulesCard({
  ...
  return (
    <div className={CARD + " p-4"} data-testid="user-rules-card">
      <div className={FIELD_LABEL}>{t("Your instructions")}</div>
      <div className={FIELD_HELP}>
        {t("Your coworkers follow these in every conversation.")}
      </div>
      <textarea
        value={draft}
      ...
      <div className="flex items-center gap-3 mt-2">
        ...
        {savedMsg && (
          <span className="text-[12.5px] text-muted">
            {t("Saved — applies to new conversations. Ones you already have open keep the instructions they started with.")}
          </span>
        )}
      </div>
    </div>
