...
export async function mockApi(page: import("@playwright/test").Page) {
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    // Re-fetch a provider's live model catalog (Configure Models "Refresh"/"Retry").
    if (/\/v1\/providers\/[^/]+\/models\/refresh$/.test(p) && m === "POST") {
      const name = p.split("/").slice(-3, -2)[0];
      const prov = providers.find((x) => x.name === name);
      if (!prov) return json({ ok: false, error: `unknown provider: ${name}` });
      if (prov.catalog?.supported) {
        prov.catalog = { supported: true, live: true, fetched_at: new Date().toISOString(), error: null, count: prov.suggested_models.length };
      }
      return json({ ok: true, provider: name, catalog: prov.catalog, suggested_models: prov.suggested_models });
    }
