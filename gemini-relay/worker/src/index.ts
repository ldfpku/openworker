...
// Do NOT gate the upstream key on its prefix: AI Studio mints both legacy `AIza…` standard
// keys and `AQ.…` auth keys (every new key since 2026). A prefix guard here once rejected a
// colleague's legitimate key (2026-08-27). The one shape still refused below is our own
// `owr_` token in the key slot — that is unambiguously a client bug, never a Google key.
